...
func TestSimulationService_UpdateSimulation(t *testing.T) {
	type fields struct {
		repository *mockRepo.IRepository
	}
	type args struct {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if err := s.UpdateSimulation(tt.args.m, tt.args.schema); (err != nil) != tt.wantErr {
				t.Errorf("SimulationService.UpdateSimulation() error = %v, wantErr %v", err, tt.wantErr)
			}
		})
	}
}

func TestSimulationService_checkAntiFraude(t *testing.T) {
	type fields struct {
		repository    *mockRepo.IRepository
		apiAntifraude *mockApi.IApiAntifraude
	}
	type args struct {
		request *dto.AntiFraudRequest
	}
	tests := []struct {
		name    string
		fields  fields
		args    args
		want    *dto.AntiFraudResponse
		wantErr bool
	}{
		// TODO: Add test cases.
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got, err := s.checkAntiFraude(tt.args.request)
			if (err != nil) != tt.wantErr {
				t.Errorf("SimulationService.checkAntiFraude() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if !reflect.DeepEqual(got, tt.want) {
				t.Errorf("SimulationService.checkAntiFraude() = %v, want %v", got, tt.want)
			}
		})
	}
}
...
func TestSimulationService_theScopeIsValid(t *testing.T) {
	type fields struct {
		repository    repository.IRepository
		sqsClient     sqsAws.Client
		apiAntifraude apiantifraude.IApiAntifraude
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got, err := s.theScopeIsValid(tt.args.escope)
			if (err != nil) != tt.wantErr {
				t.Errorf("SimulationService.theScopeIsValid() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if got != tt.want {
				t.Errorf("SimulationService.theScopeIsValid() = %v, want %v", got, tt.want)
			}
		})
	}
}
